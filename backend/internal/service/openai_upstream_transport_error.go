...
//     logs a stable warn event that alert rules can key on — see
//     logUpstreamTransportPersistentFault for why this does not touch account
//     scheduling state;
...
func (s *OpenAIGatewayService) handleOpenAIUpstreamTransportError(ctx context.Context, c *gin.Context, account *Account, err error, passthrough bool) error {
	...
	if classifyUpstreamTransportError(err).Persistent {
		logUpstreamTransportPersistentFault("service.openai_gateway", "openai.upstream_transport_error_persistent", account, safeErr)
	}
