...
func (s *RateLimitService) HandleStreamTimeout(ctx context.Context, account *Account, model string) bool {
	return s.handleStreamDeliveryFailure(ctx, account, model, "stream_timeout",
		"Stream data interval timeout for model: "+model)
}

// handleStreamDeliveryFailure 是流交付失败的共用记账逻辑：累计计数、比对阈值、
// 达阈后按配置动作标记账号。keyword/errorMessage 决定落库的归因文案。
func (s *RateLimitService) handleStreamDeliveryFailure(ctx context.Context, account *Account, model, keyword, errorMessage string) bool {
	if account == nil {
		return false
	}
	...
	slog.Info("stream_timeout_count", "account_id", account.ID, "count", count, "threshold", settings.ThresholdCount, "window_minutes", settings.ThresholdWindowMinutes, "model", model, "keyword", keyword)
...
func (s *RateLimitService) triggerStreamTimeoutTempUnsched(ctx context.Context, account *Account, settings *StreamTimeoutSettings, keyword, errorMessage string) bool {
	...
	s.notifyAccountSchedulingBlocked(account, until, keyword+"_temp_unschedulable")
	if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, reason); err != nil {
	...
	slog.Info("stream_timeout_temp_unschedulable", "account_id", account.ID, "until", until, "keyword", keyword, "error_message", errorMessage)
	return true
}

// triggerStreamTimeoutError 触发流超时错误状态
func (s *RateLimitService) triggerStreamTimeoutError(ctx context.Context, account *Account, model, errorMessage string) bool {
	errorMsg := errorMessage + " (repeated failures)"
