...
// accountReadableSnapshotJSON excludes credentials and cyclic relations. Proxy
// runtime credentials stay private: modern nodes expose only a local SOCKS URL
// through ResolveOutboundIdentity, never the raw subscription or private keys.
func accountReadableSnapshotJSON(account *Account) []byte {
	...
	if account.Proxy != nil {
		...
		proxy.Extra, _ = redactProxyExtra(account.Proxy.Extra).(map[string]any)
		clone.Proxy = &proxy
	}
