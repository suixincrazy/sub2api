...
func TestLegacySchedulerPreviousResponseYieldsToRecoveredPrimary(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	for _, batch := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			ctx := context.Background()
			svc, accounts, groupID := newOverloadScheduleFixture("false", batch)
			overloadTestTrip(svc, groupID, &accounts[0], "alias")
			require.Equal(t, accounts[1].ID, overloadTestPick(t, svc, groupID, "alias", "recovered-session"))
			svc.ObserveOpenAIAccountOverloadResult(groupID, &accounts[1], "alias", true, nil)
			responseID := "resp_fallback"
			require.NoError(t, svc.getOpenAIWSStateStore().BindResponseAccount(ctx, *groupID, responseID, accounts[1].ID, time.Hour))
			released := []int64{}
			svc.concurrencyService = NewConcurrencyService(schedulerTestConcurrencyCache{releasedIDs: &released})
			selection, decision, err := svc.SelectAccountWithSchedulerForCapability(ctx, groupID, responseID, "recovered-session", "alias", nil, OpenAIUpstreamTransportAny, OpenAIEndpointCapabilityChatCompletions, false, true, true)
			require.NoError(t, err)
			require.NotNil(t, selection)
			defer releaseLegacySchedulerDecisionSelection(selection)
			require.Equal(t, accounts[0].ID, selection.Account.ID)
			require.False(t, decision.StickyPreviousHit)
			require.Contains(t, released, accounts[1].ID, "unused fallback concurrency slot must be released")
		})
	}
}
