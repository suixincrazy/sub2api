...
func (s *OpenAIGatewayService) selectLegacyAccountByPreviousResponse(
	...
	if s.shouldYieldOpenAIOverloadSticky(groupID, account, requestedModel) || !s.openAIAccountMatchesSchedulingGroup(account, groupID) || !compatible || !scheduler.isAccountTransportCompatible(account, requiredTransport) {
		if selection.ReleaseFunc != nil {
			selection.ReleaseFunc()
		}
		return nil, false, nil
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	return scheduler.Select(ctx, OpenAIAccountScheduleRequest{
		...
		// 两个来源都要求保留绑定，取或：上游的 guardian 父账号亲和（auto-review
		// 子请求必须留在父账号上）与自有的非破坏性粘性绑定（粘性号已在本次请求的
		// 排除列表里时，逃逸不该销毁原绑定）。条件正交，任一成立就不得改绑。
		PreserveStickyBinding:   preserveGuardianParentBinding || preserveStickyBinding,
		RequirePrivacySet:       s.openAIGroupRequiresPrivacySet(ctx, groupID),
