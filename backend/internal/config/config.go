...
type GatewayConfig struct {
	...
	//
	// 这个值量的是**上游静默多久**，不是持流总共多久（2026-08-25 改）。只要上游还在出帧，
	// 窗口就不断从最后一帧有内容的 SSE data 重新起算；ping 不算内容，它只证明连接活着。
	// 旧口径从首个提交帧起算总时长，而 thinking_delta 也算提交帧，于是长思考回合的窗口在
	// 思考期间就走完、判定还没成形就把缓冲提交了——02:06:53 那一发截断正是这么漏的。
	//
