...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), codexHistoryFilter.Prepare, groupModelAllowlist, codexHistoryFilter.Apply, compositeTarget, requireGroupAnthropic, handler)`))
	...
	chains := []struct {
		...
		{group: "gateway", auth: "gin.HandlerFunc(apiKeyAuth)", marker: "gateway.Use(codexHistoryFilter.Prepare, groupModelAllowlist, codexHistoryFilter.Apply)", composite: "gateway.Use(compositeTarget)"},
	...
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), codexHistoryFilter.Prepare, groupModelAllowlist, codexHistoryFilter.Apply, compositeTarget, requireGroupAnthropic)`))
