...
// 凭证原文回填接口是全库唯一把 api_key / token 明文回传前端的路径，
// 必须留在敏感读取白名单里，否则每次查看凭证都不留痕。
func TestAccountCredentialsReadIsAudited(t *testing.T) {
	require.Equal(t,
		"admin.accounts.credentials.read",
		auditSensitiveReads["GET /api/v1/admin/accounts/:id/credentials"],
	)
}
