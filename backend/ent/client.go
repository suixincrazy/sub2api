...
type Client struct {
	...
	Proxy *ProxyClient
	// ProxySource is the client for interacting with the ProxySource builders.
	ProxySource *ProxySourceClient
...
func (c *Client) init() {
	...
	c.ProxySource = NewProxySourceClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		ProxySource:                   NewProxySourceClient(cfg),
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.Proxy, c.ProxySource, c.RedeemCode, c.RedeemCodeUsage, c.SecuritySecret,
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *ProxySourceMutation:
		return c.ProxySource.mutate(ctx, m)
...
// ProxySourceClient is a client for the ProxySource schema.
type ProxySourceClient struct {
	config
}

// NewProxySourceClient returns a client for the ProxySource from the given config.
func NewProxySourceClient(c config) *ProxySourceClient {
	return &ProxySourceClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `proxysource.Hooks(f(g(h())))`.
func (c *ProxySourceClient) Use(hooks ...Hook) {
	c.hooks.ProxySource = append(c.hooks.ProxySource, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `proxysource.Intercept(f(g(h())))`.
func (c *ProxySourceClient) Intercept(interceptors ...Interceptor) {
	c.inters.ProxySource = append(c.inters.ProxySource, interceptors...)
}

// Create returns a builder for creating a ProxySource entity.
func (c *ProxySourceClient) Create() *ProxySourceCreate {
	mutation := newProxySourceMutation(c.config, OpCreate)
	return &ProxySourceCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of ProxySource entities.
func (c *ProxySourceClient) CreateBulk(builders ...*ProxySourceCreate) *ProxySourceCreateBulk {
	return &ProxySourceCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *ProxySourceClient) MapCreateBulk(slice any, setFunc func(*ProxySourceCreate, int)) *ProxySourceCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &ProxySourceCreateBulk{err: fmt.Errorf("calling to ProxySourceClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*ProxySourceCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for ProxySource.
func (c *ProxySourceClient) Update() *ProxySourceUpdate {
	mutation := newProxySourceMutation(c.config, OpUpdate)
	return &ProxySourceUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *ProxySourceClient) UpdateOne(_m *ProxySource) *ProxySourceUpdateOne {
	mutation := newProxySourceMutation(c.config, OpUpdateOne, withProxySource(_m))
	return &ProxySourceUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *ProxySourceClient) UpdateOneID(id int64) *ProxySourceUpdateOne {
	mutation := newProxySourceMutation(c.config, OpUpdateOne, withProxySourceID(id))
	return &ProxySourceUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for ProxySource.
func (c *ProxySourceClient) Delete() *ProxySourceDelete {
	mutation := newProxySourceMutation(c.config, OpDelete)
	return &ProxySourceDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *ProxySourceClient) DeleteOne(_m *ProxySource) *ProxySourceDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *ProxySourceClient) DeleteOneID(id int64) *ProxySourceDeleteOne {
	builder := c.Delete().Where(proxysource.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &ProxySourceDeleteOne{builder}
}

// Query returns a query builder for ProxySource.
func (c *ProxySourceClient) Query() *ProxySourceQuery {
	return &ProxySourceQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeProxySource},
		inters: c.Interceptors(),
	}
}

// Get returns a ProxySource entity by its id.
func (c *ProxySourceClient) Get(ctx context.Context, id int64) (*ProxySource, error) {
	return c.Query().Where(proxysource.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *ProxySourceClient) GetX(ctx context.Context, id int64) *ProxySource {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *ProxySourceClient) Hooks() []Hook {
	hooks := c.hooks.ProxySource
	return append(hooks[:len(hooks):len(hooks)], proxysource.Hooks[:]...)
}

// Interceptors returns the client interceptors.
func (c *ProxySourceClient) Interceptors() []Interceptor {
	inters := c.inters.ProxySource
	return append(inters[:len(inters):len(inters)], proxysource.Interceptors[:]...)
}

func (c *ProxySourceClient) mutate(ctx context.Context, m *ProxySourceMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&ProxySourceCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&ProxySourceUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&ProxySourceUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&ProxySourceDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown ProxySource mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		...
		PromoCodeUsage, Proxy, ProxySource, RedeemCode, RedeemCodeUsage,
