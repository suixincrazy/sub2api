...
type UserEdges struct {
	...
	// RevokedSubscriptions holds the value of the revoked_subscriptions edge.
	RevokedSubscriptions []*UserSubscription `json:"revoked_subscriptions,omitempty"`
...
// RevokedSubscriptionsOrErr returns the RevokedSubscriptions value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) RevokedSubscriptionsOrErr() ([]*UserSubscription, error) {
	if e.loadedTypes[4] {
		return e.RevokedSubscriptions, nil
	}
	return nil, &NotLoadedError{edge: "revoked_subscriptions"}
}
...
// QueryRevokedSubscriptions queries the "revoked_subscriptions" edge of the User entity.
func (_m *User) QueryRevokedSubscriptions() *UserSubscriptionQuery {
	return NewUserClient(_m.config).QueryRevokedSubscriptions(_m)
}
