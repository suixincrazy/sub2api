...
このリポジトリは独立して保守されており、公式プロジェクトの配布チャネルではありません。公式更新はマージと検証の後に取り込み、Xray 固有のリリースは `v<公式バージョン>-xray<リビジョン>` 形式を使用し、このリポジトリからのみ更新を確認します。

## 公式版との主な違い

| 範囲 | Sub2API Xray の追加機能 |
|------|--------------------------|
...
現在の正式版は [v0.2.1-xray5](https://github.com/smmooooonn/sub2api-xray/releases/tag/v0.2.1-xray5) で、公式 `v0.2.1` とそれに対応する最新の `main` コミットに同期済みです。
...
Sub2API Xray は、AI 製品のサブスクリプションから API クォータを配分・管理するために設計された AI API ゲートウェイプラットフォームです。ユーザーはプラットフォームが生成した API キーを通じて上流の AI サービスにアクセスでき、プラットフォームは認証、課金、負荷分散、リクエスト転送を処理します。
