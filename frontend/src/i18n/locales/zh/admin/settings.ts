export default {
    settings: {
      codexHistoryFilter: {
        ...
        behavior: "移除历史 reasoning 和消息、网页搜索、代理消息及可重发工具记录的顶层 ID；修复代理明文被标记为加密内容的格式。保留本轮推理强度、call_id 配对、搜索内容及嵌套资源引用。固定 store=false，并移除 include 中的 reasoning.encrypted_content。",
        boundaries: "仅支持 HTTP/SSE。压缩上下文、服务端会话或条目引用、其他加密上下文会被明确拒绝。历史内部推理会丢弃，承载代理任务或回复正文的真实密文保留；遇到引用拒绝时，请用文本摘要开启新会话。此模式不支持浏览器直接发送 Responses 请求。",
        filteredRequests: "过滤请求数",
        removedReasoning: "移除推理项",
        removedIds: "清理记录 ID",
        normalizedAgentText: "修复代理文本片段",
