...
const baseUrlHint = computed(() => {
  ...
  if (props.account.platform === 'kimi' || props.account.platform === 'zhipu' || props.account.platform === 'deepseek') {
    return t('admin.accounts.cnProviders.baseUrlHint')
  }
...
const isGrokOAuthAccount = computed(
  () => !isUserScope.value && props.account?.platform === 'grok' && props.account?.type === 'oauth'
)
...
const submitUpdateAccount = async (accountID: number, updatePayload: Record<string, unknown>) => {
  ...
  try {
    let updatedAccount = await updateScopedAccount(accountID, withAntigravityConfirmFlag(updatePayload))
